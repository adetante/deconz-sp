...
fn decode_valid_read_parameter() {
  let frame = [ 0xa, 0xa, 0x0, 0x10, 0x0, 0x9, 0x0, 0x1, 0xf, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 ];
  let response = IncomingMessage::read(&frame);
  assert!(response.is_ok());
  let response = response.unwrap();
  assert_eq!(response.command.code(), CommandCode::ReadParameter.code(), "Invalid command in response");
  assert_eq!(response.seq, 10, "Invalid seq in response");
  assert_eq!(response.status.code(), StatusCode::Success.code(), "Invalid status in response");
  match response.payload {
    IncomingPayload::ReadParameter { parameter, value } => {
      assert_eq!(parameter.code(), ParameterCode::MacAddress.code(), "Invalid parameter");
...
fn decode_valid_write_parameter() {
  ...
  assert!(response.is_ok());
  let response = response.unwrap();
  assert_eq!(response.command.code(), CommandCode::WriteParameter.code(), "Invalid command in response");
  assert_eq!(response.seq, 10, "Invalid seq in response");
  assert_eq!(response.status.code(), StatusCode::Success.code(), "Invalid status in response");
  match response.payload {
    IncomingPayload::WriteParameter { parameter } => {
      assert_eq!(parameter.code(), ParameterCode::ApsDesignedCoordinator.code(), "Invalid parameter");
    },
    _ => panic!("Invalid response payload")
  };
}
...
fn decode_valid_device_state() {
  ...
  assert!(response.is_ok());
  let response = response.unwrap();
  assert_eq!(response.command.code(), CommandCode::DeviceState.code(), "Invalid command in response");
  assert_eq!(response.seq, 10, "Invalid seq in response");
  assert_eq!(response.status.code(), StatusCode::Success.code(), "Invalid status in response");
  match response.payload {
    IncomingPayload::DeviceState { state, .. } => {
      assert_eq!(state.code(), NetworkStateCode::Connected.code(), "Invalid parameter");
    },
    _ => panic!("Invalid response payload")
  };
}
...
fn decode_valid_change_network_state() {
  ...
  assert!(response.is_ok());
  let response = response.unwrap();
  assert_eq!(response.command.code(), CommandCode::ChangeNetworkState.code(), "Invalid command in response");
  assert_eq!(response.seq, 10, "Invalid seq in response");
  assert_eq!(response.status.code(), StatusCode::Success.code(), "Invalid status in response");
  match response.payload {
...
fn decode_valid_aps_data_indication() {
  ...
  let frame = [
    0x17, 0xa, 0x0, frame_len[0], frame_len[1], payload_len[0], payload_len[1],
    0x0,
    0x1, 0x1, 0x0, 0x0,
    0x2, 0x2, 0x0, 0x4,
    0x1, 0x0,
  ...
  assert!(response.is_ok());
  let response = response.unwrap();
  assert_eq!(response.command.code(), CommandCode::ApsDataIndication.code(), "Invalid command in response");
  assert_eq!(response.seq, 10, "Invalid seq in response");
  assert_eq!(response.status.code(), StatusCode::Success.code(), "Invalid status in response");
  match response.payload {
    IncomingPayload::ApsDataIndication { source, destination, profile_id, cluster_id, asdu, lqi, rssi } => {
      match source {
