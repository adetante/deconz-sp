...
use super::types::{ ParameterValue, Address };
use super::constants::{ CommandCode, StatusCode, ParameterCode, NetworkStateCode };
use crate::Error;
...
pub enum IncomingPayload {
  ReadParameter { parameter: ParameterCode, value: ParameterValue },
  WriteParameter { parameter: ParameterCode },
  DeviceState { state: NetworkStateCode, apsde_data_confirm: bool, apsde_data_indication: bool, configuration_changed: bool, apsde_data_request: bool },
  ChangeNetworkState { state: NetworkStateCode },
  ApsDataIndication { source: Address, destination: Address, profile_id: u16, cluster_id: u16, asdu: Vec<u8>, lqi: u8, rssi: i8 }  
}
...
fn decode_device_state(state: u8) -> Option<(NetworkStateCode,bool,bool,bool,bool)> {
  ...
  Some((network_state, apsde_data_confirm, apsde_data_indication, configuration_changed, apsde_data_request))
}
    ...
    match command {
      CommandCode::ReadParameter => {
        ...
        if input.len() < payload_len {
          return Err(Error::Decoding("Too short payload: incorrect payload length"));
        }
      ...
      CommandCode::DeviceState => {
        ...
        match decode_device_state(input[0]) {
          ...
          Some((state, apsde_data_confirm, apsde_data_indication, configuration_changed, apsde_data_request)) => 
            Ok(IncomingPayload::DeviceState { state, apsde_data_confirm, apsde_data_indication, configuration_changed, apsde_data_request })
        }
      },
      ...
      CommandCode::ApsDataIndication => {
        ...
        if input.len() < (2 + payload_length) {
          return Err(Error::Decoding("Too short payload for ApsDataIndication: invalid payload_length"));
        }
        ...
        let (source, next_offset) = match input[next_offset] {
          0x1 => {
            let address = LittleEndian::read_u16(&input[next_offset+1..next_offset+3]);
          ...
          0x3 => {
            let address = LittleEndian::read_u64(&input[next_offset+1..next_offset+9]);
        ...
        let asdu_len = LittleEndian::read_u16(&input[next_offset+4..next_offset+6]) as usize;
        ...
        Ok(IncomingPayload::ApsDataIndication { source, destination, profile_id, cluster_id, asdu, lqi, rssi })
      },
      CommandCode::DeviceStateChanged => {
        match decode_device_state(input[0]) {
          None => Err(Error::Decoding("Cannot decode device state")),
          Some((state, apsde_data_confirm, apsde_data_indication, configuration_changed, apsde_data_request)) => 
            Ok(IncomingPayload::DeviceState { state, apsde_data_confirm, apsde_data_indication, configuration_changed, apsde_data_request })
      ...
      _ => Err(Error::Decoding("This command decoder is not yet implemented"))
    }
  }
}
...
impl IncomingMessage {
  pub fn read(input: &[u8]) -> Result<Self, Error> {
    ...
    if input.len() < frame_len {
      return Err(Error::Decoding("Frame is too short: invalid Frame length value"));
    }
    match (command, status) {
      ...
      (Some(command), Some(status)) => {
        ...
        Ok(IncomingMessage { command, seq, status, payload })
      }
    }
  }
}