...
use crate::protocol::constants::{ CommandCode, ParameterCode, NetworkStateCode };
...
enum OutgoingPayload {
  Empty,
  ReadParameter { parameter: ParameterCode },
  WriteParameter { parameter: ParameterCode, value: ParameterValue },
  DeviceState,
  ChangeNetworkState { state: NetworkStateCode },
  ApsDataRequest { request_id: u8, destination: Address, profile_id: u16, cluster_id: u16, source_endpoint: u8, asdu: Vec<u8>, radius: u8 }
}
...
impl OutgoingPayload {
  ...
  fn length(&self) -> usize {
    match self {
      ...
      OutgoingPayload::ApsDataRequest { destination, asdu, ..} => {        
  ...
  fn write(&self, out: &mut [u8]) -> Result<(), Error> {
    match self {
      ...
      OutgoingPayload::ApsDataRequest { request_id, destination, profile_id, cluster_id, source_endpoint, asdu, radius } => {
        out[0] = *request_id;
        out[1] = 0x0;
...
impl OutgoingMessage {
  ...
  pub fn new_read_parameter(seq: u8, parameter: ParameterCode) -> Self {
    OutgoingMessage { command: CommandCode::ReadParameter, seq, payload: OutgoingPayload::ReadParameter { parameter } }
  }

  pub fn new_write_parameter(seq: u8, parameter: ParameterCode, value: ParameterValue) -> Self {
    OutgoingMessage { command: CommandCode::WriteParameter, seq, payload: OutgoingPayload::WriteParameter { parameter, value } }
  }

  pub fn new_device_state(seq: u8) -> Self {
    OutgoingMessage { command: CommandCode::DeviceState, seq, payload: OutgoingPayload::DeviceState }
  }

  pub fn new_change_network_state(seq: u8, state: NetworkStateCode) -> Self {
    OutgoingMessage { command: CommandCode::ChangeNetworkState, seq, payload: OutgoingPayload::ChangeNetworkState { state } }
  }

  pub fn new_aps_data_indication(seq: u8) -> Self {
    OutgoingMessage { command: CommandCode::ApsDataIndication, seq, payload: OutgoingPayload::Empty }
  }

  pub fn new_aps_data_request(seq: u8, request_id: u8, destination: Address, profile_id: u16, cluster_id: u16, source_endpoint: u8, radius: u8, asdu: Vec<u8>) -> Self {
    OutgoingMessage {
      command: CommandCode::ApsDataRequest,
      seq,
      payload: OutgoingPayload::ApsDataRequest { request_id, destination, profile_id, cluster_id, source_endpoint, asdu, radius }
    }
  }

  pub fn new_aps_data_confirm(seq: u8) -> Self {
    OutgoingMessage { command: CommandCode::ApsDataConfirm, seq, payload: OutgoingPayload::Empty }
  }

  pub fn write(&self, out: &mut [u8]) -> Result<usize, Error> {
    let frame_len = 
      if self.payload.has_variable_length() {
