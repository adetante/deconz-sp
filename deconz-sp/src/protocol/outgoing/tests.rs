...
fn encode_valid_read_parameter() {
  ...
  match request.write(&mut output) {
    Err(err) => {
      panic!("Cannot write request: {}", err)
    },
    Ok(len) => {
      println!("Frame len={} data={:x?}", len, &output);
      assert_eq!(len , 8, "Invalid frame len");
      assert_eq!(output[0], CommandCode::ReadParameter.code(), "Invalid command");
      ...
      assert_eq!(output[7], ParameterCode::ChannelMask.code(), "Invalid parameter");
    }
  }
}
...
fn encode_valid_write_parameter() {
  let request = OutgoingMessage::new_write_parameter(10, ParameterCode::NwkAddress, ParameterValue::U16(333));
  let mut output = [0; 32];
  match request.write(&mut output) {
    Err(err) => {
      panic!("Cannot write request: {}", err)
    ...
    Ok(len) => {
      ...
      assert_eq!(output[0], CommandCode::WriteParameter.code(), "Invalid command");
      assert_eq!(output[1], 10, "Invalid sequence");
      assert_eq!(output[2], 0);
      let frame_len = LittleEndian::read_u16(&output[3..5]);
      ...
      assert_eq!(output[7], ParameterCode::NwkAddress.code(), "Invalid parameter");
...
fn encode_valid_network_state() {
  ...
  match request.write(&mut output) {
    ...
    Ok(len) => {
      println!("Frame len={} data={:x?}", len, &output);
      assert_eq!(len , 8, "Invalid frame len");
      assert_eq!(output[0], CommandCode::DeviceState.code(), "Invalid command");
      assert_eq!(output[1], 10, "Invalid sequence");
      assert_eq!(output[2], 0);
      let frame_len = LittleEndian::read_u16(&output[3..5]);
      assert_eq!(frame_len, 8, "Invalid fame len");
...
fn encode_valid_change_network_state() {
  ...
  match request.write(&mut output) {
    ...
    Ok(len) => {
      ...
      assert_eq!(output[0], CommandCode::ChangeNetworkState.code(), "Invalid command");
      assert_eq!(output[1], 10, "Invalid sequence");
      assert_eq!(output[2], 0);
      let frame_len = LittleEndian::read_u16(&output[3..5]);
...
fn encode_valid_aps_data_indication() {
  ...
  match request.write(&mut output) {
    ...
    Ok(len) => {
      ...
      assert_eq!(output[0], CommandCode::ApsDataIndication.code(), "Invalid command");
      assert_eq!(output[1], 10, "Invalid sequence");
      assert_eq!(output[2], 0);
      let frame_len = LittleEndian::read_u16(&output[3..5]);
...
fn encode_valid_aps_data_request_with_nkw() {
  ...
  let request = OutgoingMessage::new_aps_data_request(10, 100, Address::NWK(1,2), 10, 11, 12, 13, asdu.clone());
  ...
  match request.write(&mut output) {
    ...
    Ok(len) => {
      ...
      assert_eq!(output[0], CommandCode::ApsDataRequest.code(), "Invalid command");
      assert_eq!(output[1], 10, "Invalid sequence");
      assert_eq!(output[2], 0);
      ...
      assert_eq!(output[9], DestinationMode::NWK.code(), "Invalid destination mode");
...
fn encode_valid_aps_data_request_with_group() {
  let asdu: Vec<u8> = (0..255).collect();
  let request = OutgoingMessage::new_aps_data_request(10, 100, Address::Group(1), 10, 11, 12, 13, asdu.clone());
  let mut output = [0; 300];
  match request.write(&mut output) {
    Err(err) => {
      panic!("Cannot write request: {}", err)
    ...
    Ok(len) => {
      ...
      assert_eq!(output[7], 100, "Invalid request_id");
      assert_eq!(output[8], 0, "Invalid flag");
      assert_eq!(output[9], DestinationMode::Group.code(), "Invalid destination mode");
      let destination = LittleEndian::read_u16(&output[10..12]);
      assert_eq!(destination, 1, "Invalid destination");
...
fn encode_valid_aps_data_request_with_ieee() {
  let asdu: Vec<u8> = (0..255).collect();
  let request = OutgoingMessage::new_aps_data_request(10, 100, Address::IEEE(1,2), 10, 11, 12, 13, asdu.clone());
  let mut output = [0; 300];
  match request.write(&mut output) {
    Err(err) => {
      panic!("Cannot write request: {}", err)
    ...
    Ok(len) => {
      ...
      assert_eq!(output[7], 100, "Invalid request_id");
      assert_eq!(output[8], 0, "Invalid flag");
      assert_eq!(output[9], DestinationMode::IEEE.code(), "Invalid destination mode");
...
fn encode_valid_aps_data_confirm() {
  ...
  match request.write(&mut output) {
    ...
    Ok(len) => {
      println!("Frame len={} data={:x?}", len, &output);
      assert_eq!(len , 7, "Invalid frame len");
      assert_eq!(output[0], CommandCode::ApsDataConfirm.code(), "Invalid command");
      assert_eq!(output[1], 10, "Invalid sequence");
      assert_eq!(output[2], 0);
      let frame_len = LittleEndian::read_u16(&output[3..5]);
      assert_eq!(frame_len, 7, "Invalid fame len");
